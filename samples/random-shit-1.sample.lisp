...
(labels ((fibonacci      (n k) (fibonacci/iter n 0 1 k))
         (fibonacci/iter (n a b k)
           (if (= n 0)
               (k a)
               (if (= n 1)
                   (k b)
                   (fibonacci/iter (- n 1) b (+ a b) k)))))
  (fibonacci 10 (lambda (x) x)))

;; this causes problems for some versions of closure conversion
(labels ((compose-1/1 (f g) (lambda (x) (f (g x))))
         (compose-1/1/2 (h f g) (lambda (x y) (h (f (g x y)))))
         (testfn (x)
           (labels ((bluh1 (y) (bluh2 x y))
                    (bluh2 (y z) (+ x y z))
                    (bluh3 (new-x y) (set! x new-x) (bluh1 y))
                    (bluh4 (x) (if (< x 10) (set! bluh4 bluh1)) (bluh4 (- x 1)))
                    (even (y) (if (= y 0) x (odd (- y 1))))
                    (odd (y) (if (= y 1) (+ x 1) (even (- x 1)))))
             (bluh1 1)
             (bluh2 1 2)
             (bluh4 100)
             (if (< (bluh2 3 4) 5)
              (compose-1/1 even odd)
              (compose-1/1/2 bluh3 bluh1 bluh2)))))
  ((testfn 3) 5))
