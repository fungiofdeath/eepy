  ...
  switch (exp.$) {
    ...
    case 'record':
      return {
        ...
        args: exp.items.map(parse_tree_to_ast),
        span: exp.span,
      };
    case 'list': {
      ...
      return convert_form(exp, exp.items[0], exp.items.slice(1));
    }
    case 'infix':
...
function convert_form(exp, fn, args) {
  const ast = x => parse_tree_to_ast(x);
  const ast_opt = (x, default_span) => (!x ? nil(default_span) : ast(x));
  const ast_block = items => {
    if (items.length === 0) {
  ...
  switch (fn.name) {
    ...
    case 'block': {
      ...
      return { $: 'block', subforms: args.map(ast), span: exp.span };
    }
    case 'set!': {
      if (args.length !== 2) {
        throw new NodeError(
    ...
    case 'if': {
      ...
      return {
        ...
        cond: ast(args[0]),
        then: ast(args[1]),
        otherwise: ast_opt(args[2], exp.span),
      };
    }
    case 'let':
    case 'let*':
    case 'letrec*': {
          ...
          if (bind.$ === 'atom') {
            ...
            if (bind.items.length < 2) {
              ...
              return {
                ...
                value: ast_block(bind.items.slice(1)),
                span: bind.span,
              };
            }
          } else {
      ...
      const body = ast_block(args.slice(1));
            ...
            if (bind.items.length < 3) {
              ...
              return {
                ...
                value: {
                  ...
                  params,
                  span: bind.span,
                  body: ast_block(bind.items.slice(2)),
                },
                span: bind.span,
              };
            }
